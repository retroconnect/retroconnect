...
extern "C" {
    ...
    void hidh_callback(void *handler_args, esp_event_base_t base, int32_t id, void *event_data)
    {
        ...
        case ESP_HIDH_INPUT_EVENT: {
            ...
            //Button press info is in the following:
            ESP_LOG_BUFFER_HEX(TAG, param->input.data, param->input.length);

            //printf("LENGTH: %d", (int)param->input.length);
            if ((int)param->input.length == 1 && (unsigned int)*(param->input.data) < 2){//&& ((unsigned char*)(param->input.data)) == 1) {
                home_button_handler((unsigned int)*(param->input.data));
                break;
            }

            //Send button inputs to main program
            //param->input.data is the HID data (16 byte button_struct_t)
            parse_inputs((button_struct_t*)(param->input.data));
